...
'''
    서울시 공공데이터 API 추출 시, 특정 날짜 컬럼을 조사하여 배치 날짜 기준 전날 데이터가 존재하는지 체크하는 센서 
    <가정>
    1. 데이터셋에 날짜 컬럼이 존재해야 함 
    2. API 사용 시, 데이터가 해당 날짜 컬럼으로 ORDER BY DESC 되어 있어야 함
'''
...
class SeoulApiDateSensor(BaseSensorOperator):
    ...
    def __init__(self, dataset_nm, base_dt_col, day_off = 0, **kwargs):
        '''
            ...
            day_off: 배치일 기준으로 생성 여부를 확인하고자 하는 날짜 차이를 입력 (기본값: 0)
        '''
        super().__intit__(**kwargs)
    ...
    def poke(self, context):
        ...
        last_dt = row_data[0].get(self.base_dt_col) # 첫 번째 행(row)의 base_dt_col 값
