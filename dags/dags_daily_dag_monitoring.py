        ...
        with closing(postgres_hook.get_conn()) as conn:
            with closing(conn.cursor()) as cursor:
                with open('/opt/airflow/files/sqls/daily_dag_monitoring.sql', 'r') as sql_file:
                    ...
                    query = '\n'.join(sql_file.readlines()) # SQL 파일 내 모든 쿼리문을 읽어옴
                    cursor.execute(query)
                    result = cursor.fetchall()
                    result = pd.DataFrame(result)
                    result.columns = [
                        'dag_id', 
                        'run_cnt', 'success_cnt', 'failed_cnt', 'running_cnt', 
                        'last_failed_date', 'last_success_date', 
                    ...
                    # 2) 미수행 대상
                    skipped_df = result.query("(run_cnt == 0)")
                    ...
                    # 3) 수행 중 대상
                    running_df = result.query("(running_cnt > 0)")
                    ...
                    # 4) 성공 대상
                    done_success_cnt = result.query("(failed_cnt == 0) and (run_cnt > 0) and (running_cnt == 0)").shape[0]
                    yesterday = pendulum.yesterday('Asia/Seoul').strftime('%Y-%m-%d')
                    now = pendulum.now('Asia/Seoul').strftime('%Y-%m-%d %H:%M:%S')
                    return_blocks = [
                        sb.section_text(f"DAG 수행현황 알람({yesterday} ~ {now})"),
                        sb.divider(),
