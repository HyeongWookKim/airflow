...
def get_prompt_for_chatgpt(yyyymmdd, market, cnt):
    ...
    return_prompt_lst = []
    df_ohlcv = stock.get_market_ohlcv(date = yyyymmdd, market = market) # (market: KOSPI/KOSDAQ/KONEX/ALL)
    df_fund = stock.get_market_fundamental(yyyymmdd, market = market)

    df_total = pd.merge(df_ohlcv, df_fund, how = 'inner', on = '티커')
    df_total.sort_values(by = ['등락률'], ascending = False, inplace = True)
